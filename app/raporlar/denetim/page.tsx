...
export default async function DenetimRaporuPage({ searchParams }: PageProps) {
  ...
  return (
    <AppShell selectedHotelId={otelId} hotelSelectorAction="/raporlar/denetim">
      <div className="grid gap-6">
        <div>
          <div className="text-sm text-muted">Raporlar &gt; Denetim</div>
          <h1 className="mt-1 text-2xl font-semibold">Denetim Raporu</h1>
        </div>
        <form action="/raporlar/denetim" className="flex flex-wrap items-end gap-2 rounded-md border border-border bg-surface p-4">
          ...
          <ExportButtons filters={{ otelId, kullaniciId, islemTuru, baslangic, bitis }} reportType="denetim" />
        </form>
        <section className="rounded-md border border-border bg-surface p-4">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-lg font-semibold">Log Kayitlari</h2>
